...
-- activate on current $SHELL
local function source_activate(activate_file)
	return "source " .. plugin_path() .. "/bin/" .. activate_file
end

local function source_string()
	local user_shell = os.getenv("SHELL") or "bash"
	local shell = user_shell:gmatch("([^/]*)$")()
	if shell == "fish" then
		return source_activate("activate.fish")
	elseif shell == "csh" then
		return source_activate("activate.csh")
	end
	return source_activate("activate")
end

nvim_rocks.install = function(name)
	rock_print('Trying to install "' .. name .. '"')
	local result = vim.fn.systemlist(source_string() .. "&&  luarocks install " .. name)
	print_list(result)
...
nvim_rocks.remove = function(name)
	...
	local result = vim.fn.systemlist(source_string() .. "&&  luarocks remove " .. name)
	print_list(result)
...
nvim_rocks.list = function(simple, outdated)
	local cmd_output
	if outdated then
		cmd_output = vim.fn.systemlist(source_string() .. "&& luarocks list --porcelain --outdated")
	else
		cmd_output = vim.fn.systemlist(source_string() .. "&& luarocks list --porcelain")
	end
	local result = {}
